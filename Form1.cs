...
namespace OnlineHull
{
    public partial class Form1 : Form
    {
        ...
        void InsertionHull(PointF point)
        {
            if (p.Size == 0)
                p.Insert(point);
            ...
            {
                Console.WriteLine("HEYYYYYY! I'm HERE!!!");
            }
        }
        ...
        {
            if (p.Size == 1)
                return s.X == p.Point.X && s.Y == p.Point.Y; ;//(Math.Abs(s.X - p.Point.X) < 0.0001) && (Math.Abs(s.Y - p.Point.Y) < 0.0001);
            if (p.Size == 2)
            {
        ...
        IntersectionType LinesIntersect(PointF a, PointF b, PointF c, PointF d)
        {
            bool intersects = PointsOnDifferentSides(a, b, c, d) && PointsOnDifferentSides(c, d, a, b);
            if (intersects)
            {
                if (a.Y == c.Y || b.Y == c.Y || a.Y == d.Y || b.Y == d.Y)
                    return IntersectionType.VertexIntersects;
                return IntersectionType.Intersects;
            }
            return IntersectionType.NotIntersects;
        }
